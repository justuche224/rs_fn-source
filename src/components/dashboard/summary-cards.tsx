  ...
  async function getInvestments() {
    try {
      ...
      const totalInvestmentAmount = Object.values(
        investments.totalInvestedAmountByCurrency
        ...
        <CardContent>
          {investmentState.loading ? (
            ...
            <>
              <div className="text-2xl font-bold">
                $
                {investmentState.data?.totalInvestmentAmount.toFixed(2) ||
                  "0.00"}
              </div>
              <p className="text-xs text-muted-foreground">
                <span className="font-medium">
                  {investmentState.data?.totalCount || 0}
                </span>{" "}
                investment(s)
              </p>
            </>
        ...
        <CardContent>
          {referralState.loading ? (
            ...
            <>
              ...
              <p className="text-xs text-muted-foreground">
                <span className="font-medium">
                  {referralState.data?.totalReferrals || 0}
                </span>{" "}
                referral(s)
              </p>
            </>
